...
import { getResultDataList } from '../../util/preview';
...
export const setTemplateEvent = async (previewIdChar = '') => {
    ...
    console.log('resultDataList', getResultDataList());
    const resultDataList = getResultDataList();
    ...
    resultTableObjList.forEach((resultTableObj: HTMLDivElement) => {
        ...
        const qPageInfoList = objects.getResultQPageList(resultTableObj);
        const resultType = objects.getObjResultTplType(resultTableObj);
        const resultFieldList = objects.getObjResultFieldList(resultTableObj);
