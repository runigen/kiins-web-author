...
import { ETemplete_Object_Type, ETemplateType } from '../../const/types';
...
export const setTemplateEvent = () => {
    ...
    tplPointList.forEach((element: any) => {
        if (
            element.getAttribute('tpl-object-type') === 'end-point' ||
            element.getAttribute('tpl-object-type') === 'answer'
        ) {
...
const setLineTemplateEvent_Click = (e: MouseEvent) => {
    ...
    if (selectPoint.getAttribute('tpl-object-type') === 'answer') {
        selectPoint = previewContainer.querySelector(
            `.object[tpl-object-type="end-point"][tpl-ano="${aNo}"]`,
        ) as HTMLDivElement;
    }
